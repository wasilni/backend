...
use App\Notifications\GenteleReminderNotification;
...
if($driver->user->lang=='en'){
       $title ='Gentle Reminder 😊️';
        $body = 'Please open the'.' '.env('APP_NAME').' app to get ride requests';
    
}
else{
       $title ='تذكير راقي' ;
        $body ='رجاء افتح تطبيق '.' '.env('APP_NAME').' لتستلبم طلباتك    ';
}
        ...
        $title_ar ='تذكير راقي' ;
        $body_ar ='رجاء افتح تطبيق '.' '.env('APP_NAME').' لتستلبم طلباتك    ';

        $title_en ='Gentle Reminder 😊️';
        $body_en = 'Please open the'.' '.env('APP_NAME').' app to get ride requests';
        
        
 $notifable_driver->notify(new GenteleReminderNotification($title_ar,$title_en, $body_ar,$body_en));
