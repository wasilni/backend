...
use App\Notifications\AppendAccountNotification;
...
class DriverDocumentController extends BaseController
{
    ...
    public function toggleApprove(Driver $driver, $status)
    {
       ...
       $title =' Approved your account' ;
        $body = 'Approved your account now ';
         $message = 'transfere point Successfully';
    
       }
    else{
        $title = 'تم تفعيل الحساب تطبيق وصلنى';
        $body = 'تم تفعيل حسابك بنجاح  ';
          $message = 'تم تفعيل الحساب بنجاح';
            ...
            $socket_success_message = PushEnums::DRIVER_ACCOUNT_APPROVED;
               ...
               $title_ar = 'تم تفعيل الحساب تطبيق وصلنى';
        $body_ar = 'تم تفعيل حسابك بنجاح  ';
        $title_en = 'approved your account' ;
        $body_en =  'approved your account Successfully';
      
                  $driver->user->notify(new AppendAccountNotification($title_ar,$title_en,$body_ar, $body_en));
