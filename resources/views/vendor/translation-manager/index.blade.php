    ...
    <div class="wrapper skin-blue-light">
        ...
        <div class="content-wrapper">
            <section class="content">
                <div class="box">
                    <div class="box-body">
                        <p>{{trans('pages_names.Warning_translations')}}</code> .</p>
                                        ...
                                        <div class="row">
                                            <div class="col-sm-3">
                                                <select name="replace" class="form-control">
                                                    <option value="0">
                    {{trans('pages_names.Append_new_translations')}}</option>
                                                    <option value="1">
                      {{trans('pages_names.Replace_existing_translations')}}
                                                       </option>
                                                </select>
                                            </div>
                                            <div class="col-sm-2">
                                                <button type="submit" class="btn btn-success btn-sm" data-disable-with="Loading..">
                                       {{trans('pages_names.Import_groups')}} </button>
                                            </div>
                                        </div>
                                    </div>
                                </form>
                                <form class="form-find" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
                                    <div class="form-group">
                                        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                        <button type="submit" class="btn btn-info btn-sm" data-disable-with="Searching..">
           {{trans('pages_names.Find_translations_in_files')}}
                                           </button>
                                    </div>
                                </form>
                            <?php endif; ?>
                            <?php if (isset($group)) : ?>
                                <form class="form-inline form-publish" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?php echo $group ?>? This will overwrite existing language files.">
                                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                    <button type="submit" class="btn btn-info btn-sm mr-4" data-disable-with="Publishing..">
                    {{trans('pages_names.Publish_translations')}}
                                        
                                        </button>
                <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="btn btn-danger btn-sm">{{trans('pages_names.Back')}}
                                        </a>
                                </form>
                            <?php endif; ?>
                        </p>
                        <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
                            ...
                            <div class="form-group">
                                <p>
                         {{trans('pages_names.Choose_group_display')}}
                                    </p>
                            ...
                            <div class="form-group">
                                <label> 
                                {{trans('pages_names.Enter_new_group')}}
                                  </label>
                                <input type="text" class="form-control" name="new-group" />
                            </div>
                            <div class="form-group">
                                <input type="submit" class="btn btn-default btn-sm" name="add-group" value="{{trans('pages_names.Add_and_edit_keys')}}" />
                            </div>
                        </form>
                        <?php if ($group) : ?>
                            <form action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST" role="form">
                                ...
                                <div class="form-group">
                                    ...
                                    <label>
                       {{trans('pages_names.Add_new_keys_to_this_group')}}
                                    </label>
           <textarea class="form-control" rows="3" name="keys" placeholder=" {{trans('pages_names.Add_key_perline')}}
                                    "></textarea>
                                </div>
                                <div class="form-group">
                                    <input type="submit" value="{{trans('pages_names.Add_keys')}}" class="btn btn-primary btn-sm">
                                </div>
                            </form>
                            <div class="row">
                                <div class="col-sm-2">
                                    <span class="btn btn-default btn-sm enable-auto-translate-group">
                         {{trans('pages_names.Use_Auto_Translate')}}
                                        </span>
                                </div>
                            </div>
                            <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postTranslateMissing') ?>">
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <label for="base-locale">
             {{trans('pages_names.Base_Locale_for_Auto_Translations')}}
                                               </label>
                           ...
                           {{trans('pages_names.Enter_target_locale_key')}}
                                                </label>
                                            <input type="text" name="new-locale" class="form-control" id="new-locale" placeholder="{{trans('pages_names.Enter_target_locale_key')}}" />
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <button type="submit" class="btn btn-default btn-sm" data-disable-with="Adding..">
   {{trans('pages_names.Auto_translate_missing_translations')}}
                                                </button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <?php else : ?>
                            <fieldset>
                                <legend> {{trans('pages_names.Supported_locales')}}
                                    </legend>
                                <p>
                                    
                      {{trans('pages_names.Current_supported_locales')}}
                                ...
                                <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddLocale') ?>">
                                    ...
                                    <div class="form-group">
                                        <p>
                      {{trans('pages_names.Enter_new_locale_key')}}
                                        </p>
                                        <div class="row">
                                            <div class="col-sm-3">
                                            ...
                                            <div class="col-sm-2">
                                                <button type="submit" class="btn btn-default btn-sm" data-disable-with="Adding..">   
                                 {{trans('pages_names.Add_new_locale')}}
                                                    </button>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <fieldset>
                  <legend>  {{trans('pages_names.Export_all_translations')}}
                                    </legend>
                                <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                    <button type="submit" class="btn btn-primary btn-sm" data-disable-with="Publishing..">{{trans('pages_names.Publish_all')}}
                                        </button>
                                </form>
                            </fieldset>
