...
use App\Notifications\NewTripTriNotification;
...
if($driver->user->lang=='en'){
       $title ='new request trip' ;
        $body = 'Trip started towards the drop location';
    
}
else{
       $title ='طلب رحلة جديد' ;
        $body = 'طلب رحلة جديد تجاه موقعك';
}
  ...
  $title_en ='new request trip' ;
        $title_ar ='طلب رحلة جديد' ;

        $body_en = 'Trip started towards the drop location';
                $body_ar = 'طلب رحلة جديد تجاه موقعك';
        
        
 $notifable_driver->notify(new NewTripTriNotification($title_ar,$title_en, $body_ar,$body_en));
